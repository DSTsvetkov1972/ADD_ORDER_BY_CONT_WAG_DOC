...
def insert_from_df(dwh_table_name, df, operation_name):
        ...
        print(
            Fore.GREEN +
            f'Закончен процесс для: {operation_name} (начали {thread_start_time}, закончили {thread_finish_time})' +
            Fore.WHITE)
...
def connection_settings_file_creator(CLICK_HOST, CLICK_PORT, CLICK_DBNAME, CLICK_USER, CLICK_PWD):
    try: 
        ...
        encoded_text = Fernet(
             b'lXgjsyWLG2R-nAWC1vBkz-FWFzeWFi-71rNMiO2ON40=').encrypt(params.encode('utf-8'))
        if os.path.isfile(config_file):
            win32api.SetFileAttributes(
                 config_file, win32con.FILE_ATTRIBUTE_NORMAL)
